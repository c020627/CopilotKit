...
export interface NotificationCohort {
  id: string;
  name: string;
  description: string;
...
function id(value: unknown): value is string {
  return text(value, 64) && /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value);
}
...
export function parseNotificationFeed(value: unknown): NotificationFeed | null {
  if (
    !record(value) ||
    !onlyKeys(value, ["schemaVersion", "cohorts", "notifications"]) ||
  ...
  if (
    ...
    value.cohorts.length > 100 ||
    value.notifications.length > 100
  )
    return null;
  const cohorts: NotificationCohort[] = [];
  for (const cohort of value.cohorts) {
    if (
      !record(cohort) ||
      !onlyKeys(cohort, ["id", "name", "description", "conditions"]) ||
      !id(cohort.id) ||
      !text(cohort.name) ||
      !text(cohort.description, 1000) ||
      !validNotificationConditions(cohort.conditions)
    )
      return null;
    if (cohorts.some((c) => c.id === cohort.id)) return null;
    cohorts.push({
      id: cohort.id,
      name: cohort.name,
  ...
  for (const notice of value.notifications) {
    ...
    if (
      !ids(notice.cohorts) ||
      !notice.cohorts.length ||
      notice.cohorts.some((key) => !cohorts.some((c) => c.id === key))
    )
      return null;
    if (
  ...
  return { schemaVersion: 1, cohorts, notifications };
}
...
export function parseNotificationState(
  ...
  if (
    ...
    (value.activeId !== null && !id(value.activeId))
  )
    return null;
  return {
    schemaVersion: 1,
...
export function reconcileNotifications(
  ...
  const eligible = feed.notifications.filter(
    (n) => matchNotification(n, feed, context).matches,
  );
...
export function matchNotification(
  ...
  feed: NotificationFeed,
  context: NotificationContext,
): { matches: boolean; cohorts: string[]; reasons: string[] } {
  if (!context.development)
    return { matches: false, cohorts: [], reasons: ["Development only"] };
  if (
    !context.sdkVersion ||
  ...
  const cohorts = notice.cohorts.filter((id) => {
    const cohort = feed.cohorts.find((c) => c.id === id);
    if (!cohort) {
      reasons.push(`Unknown cohort: ${id}`);
      return false;
    }
    return Object.entries(cohort.conditions).every(([field, expected]) => {
      ...
      if (!matches)
        reasons.push(
          `${id}: ${field} ${actual === undefined ? "is unknown" : `requires ${expected} (got ${actual})`}`,
        );
      return matches;
    });
  });
  return {
    matches: cohorts.length > 0,
    cohorts,
    reasons: cohorts.length ? [] : reasons,
  };
}
